...
# ─────────────────────────────────────────────────────────────
# PASO 8: Filtro interactivo
# ─────────────────────────────────────────────────────────────
# Permite filtrar los elementos de una capa vectorial según el
# valor de un atributo (numérico -> slider de rango,
# categórico -> selección múltiple). El resultado se usa luego
# en vez de la capa original al momento de dibujarla en el mapa.

st.sidebar.markdown("---")
st.sidebar.subheader("🎚️ Filtro interactivo")

capas_render = dict(capas)  # copia de trabajo: aquí se guardan las capas ya filtradas
opciones_filtro = ["(Ninguna)"] + capas_activas
capa_filtro = st.sidebar.selectbox("Capa a filtrar", opciones_filtro, key="filtro_capa")

if capa_filtro != "(Ninguna)":
    gdf_original = capas[capa_filtro]
    # Solo columnas de atributos (se excluye la geometría)
    cols_attr = [c for c in gdf_original.columns if c != gdf_original.geometry.name]

    if cols_attr:
        col_filtro = st.sidebar.selectbox("Atributo", cols_attr, key="filtro_col")
        serie = gdf_original[col_filtro]

        if np.issubdtype(serie.dtype, np.number):
            # Atributo numérico -> slider de rango
            val_min, val_max = float(serie.min()), float(serie.max())
            if val_min == val_max:
                st.sidebar.caption(f"Todos los valores de '{col_filtro}' son iguales ({val_min}).")
            else:
                rango = st.sidebar.slider(
                    f"Rango de {col_filtro}",
                    min_value=val_min, max_value=val_max,
                    value=(val_min, val_max), key="filtro_rango",
                )
                gdf_filtrado = gdf_original[
                    (serie >= rango[0]) & (serie <= rango[1])
                ]
                capas_render[capa_filtro] = gdf_filtrado
        else:
            # Atributo categórico -> selección múltiple
            valores_unicos = sorted(serie.dropna().astype(str).unique().tolist())
            seleccion = st.sidebar.multiselect(
                f"Valores de {col_filtro}",
                valores_unicos, default=valores_unicos, key="filtro_valores",
            )
            gdf_filtrado = gdf_original[serie.astype(str).isin(seleccion)]
            capas_render[capa_filtro] = gdf_filtrado

        st.sidebar.caption(
            f"Mostrando {len(capas_render[capa_filtro])} de {len(gdf_original)} elementos."
        )
    else:
        st.sidebar.caption("Esta capa no tiene atributos filtrables.")

# ─────────────────────────────────────────────────────────────
# PASO 9: Panel de estadísticas
# ─────────────────────────────────────────────────────────────
# Estadísticas descriptivas de la capa seleccionada: conteo,
# área total (polígonos), longitud total (líneas) y
# min/max/promedio de un atributo numérico. Se calcula sobre
# la capa ya filtrada (capas_render) para que el panel refleje
# lo que efectivamente se ve en el mapa.

st.sidebar.markdown("---")
st.sidebar.subheader("📊 Panel de estadísticas")

opciones_stats = ["(Ninguna)"] + capas_activas
capa_stats = st.sidebar.selectbox("Capa a analizar", opciones_stats, key="stats_capa")

if capa_stats != "(Ninguna)":
    gdf_stats = capas_render.get(capa_stats, capas[capa_stats])

    if len(gdf_stats) == 0:
        st.sidebar.warning("La capa (filtrada) no tiene elementos.")
    else:
        st.sidebar.write(f"**Elementos:** {len(gdf_stats)}")

        # Proyectar a un CRS métrico (UTM 18S, adecuado para la
        # zona de Paillaco/Valdivia) para calcular área/longitud reales.
        try:
            gdf_metrico = gdf_stats.to_crs(epsg=32718)
            geom_tipo = gdf_metrico.geom_type.iloc[0] if len(gdf_metrico) else ""

            if "Polygon" in geom_tipo:
                area_total_ha = gdf_metrico.geometry.area.sum() / 10_000
                st.sidebar.write(f"**Área total:** {area_total_ha:,.2f} ha")
            elif "Line" in geom_tipo:
                long_total_km = gdf_metrico.geometry.length.sum() / 1_000
                st.sidebar.write(f"**Longitud total:** {long_total_km:,.2f} km")
        except Exception as e:
            st.sidebar.caption(f"No fue posible calcular área/longitud: {e}")

        # Estadísticas de un atributo numérico a elección
        cols_num = [
            c for c in gdf_stats.columns
            if c != gdf_stats.geometry.name and np.issubdtype(gdf_stats[c].dtype, np.number)
        ]
        if cols_num:
            col_stats = st.sidebar.selectbox("Atributo numérico", cols_num, key="stats_col")
            serie_stats = gdf_stats[col_stats].dropna()
            if len(serie_stats) > 0:
                c1, c2 = st.sidebar.columns(2)
                c1.metric("Mínimo", f"{serie_stats.min():,.2f}")
                c2.metric("Máximo", f"{serie_stats.max():,.2f}")
                st.sidebar.metric("Promedio", f"{serie_stats.mean():,.2f}")
        else:
            st.sidebar.caption("Esta capa no tiene atributos numéricos.")

# ─────────────────────────────────────────────────────────────
# PASO 10: Geocodificador
# ─────────────────────────────────────────────────────────────
# Campo de búsqueda de direcciones/lugares que centra el mapa
# automáticamente en el resultado, usando geopy (Nominatim/OSM).

st.sidebar.markdown("---")
st.sidebar.subheader("🔍 Geocodificador")

if "geocod_resultado" not in st.session_state:
    st.session_state["geocod_resultado"] = None

direccion_busqueda = st.sidebar.text_input(
    "Buscar dirección o lugar", placeholder="Ej: Paillaco, Los Ríos, Chile", key="geocod_input"
)

col_buscar, col_limpiar = st.sidebar.columns(2)
buscar_click = col_buscar.button("Buscar", key="geocod_btn")
limpiar_click = col_limpiar.button("Limpiar", key="geocod_clear_btn")

if limpiar_click:
    st.session_state["geocod_resultado"] = None

if buscar_click and direccion_busqueda.strip():
    try:
        geolocator = Nominatim(user_agent="geovisualizador_paillaco_uach")
        with st.spinner("Buscando ubicación..."):
            ubicacion = geolocator.geocode(direccion_busqueda, timeout=10)
        if ubicacion:
            st.session_state["geocod_resultado"] = {
                "lat": ubicacion.latitude,
                "lon": ubicacion.longitude,
                "direccion": ubicacion.address,
            }
            st.sidebar.success(f"📍 {ubicacion.address}")
        else:
            st.session_state["geocod_resultado"] = None
            st.sidebar.warning("No se encontró esa dirección o lugar.")
    except Exception as e:
        st.sidebar.error(f"Error al geocodificar: {e}")
...
zoom_inicial = 11
resultado_geocod = st.session_state.get("geocod_resultado")
if resultado_geocod:
    centro = [resultado_geocod["lat"], resultado_geocod["lon"]]
    zoom_inicial = 15

m = folium.Map(location=centro, zoom_start=zoom_inicial, tiles="OpenStreetMap")
folium.TileLayer("CartoDB positron",    name="Mapa claro").add_to(m)
folium.TileLayer("CartoDB dark_matter", name="Mapa oscuro").add_to(m)

if resultado_geocod:
    folium.Marker(
        location=[resultado_geocod["lat"], resultado_geocod["lon"]],
        popup=resultado_geocod["direccion"],
        tooltip="📍 Resultado de búsqueda",
...
for nombre in capas_activas:
    gdf        = capas_render.get(nombre, capas[nombre])
    nombre_low = nombre.lower()
